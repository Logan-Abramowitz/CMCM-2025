...
def build_smart_cover_route(G_sub, start, full_G=None, uturn_penalty=1000.0):
    """
    Smarter continuous traversal:
      - Penalizes U-turns heavily
      - Prefers unvisited edges, but can reuse covered edges if that avoids a U-turn
      - Optionally consults the full_G (whole city graph) to cross clusters when stuck
    """
    visited_edges = set()
    route_edges = []
    u = start
    ...
    while True:
        unvisited = [v for v in G_sub.neighbors(u) if edge_key(u, v) not in visited_edges]
        candidates = []
        for v in G_sub.neighbors(u):
            ang = turn_alignment(prev, u, v)
            penalty = 0.0
            if ang < -0.9:  # U-turn (~180°)
                penalty += uturn_penalty
            # prefer unvisited
            visited_cost = 0 if edge_key(u, v) not in visited_edges else 50.0
            cost = visited_cost + penalty - (ang * 10)
            candidates.append((cost, v))
        if not candidates:
            # try to reach nearest unvisited edge via existing graph
            ...
            try:
                ...
                if reachable:
                    ...
                    u = next_node
                    continue
                # if completely stuck, use full_G to bridge clusters
                if full_G is not None:
                    all_nodes = list(full_G.nodes)
                    next_node = min(all_nodes, key=lambda n: Point(u).distance(Point(n)))
                    path = nx.shortest_path(full_G, u, next_node, weight="time")
                    for a, b in zip(path[:-1], path[1:]):
                        route_edges.append((a, b))
                    u = next_node
                    continue
...
def cluster_and_build_routes_connected(G, n_vehicles=10):
    ...
    for i in range(n_vehicles):
        ...
        route = build_smart_cover_route(H, start, full_G=G)
